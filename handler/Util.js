...
class Util {
  constructor() {
    ...
    this.fetchText = function fetchText(url, options) {
    ...
    this.shortener = function shortener(url) {
      this.fetchText(`https://tinyurl.com/api-create.php?url=${url}`)
      .then((text) => resolve(text))
    ...
    this.charReplace = function charReplace(input, obj) {
    ...
    this.ping = function ping(timestamp, now) {

      return moment.duration(now - moment(timestamp * 1000)).asSeconds();

    }

    this.downloadMedia = function downloadMedia(url, dest, cb) {

      let typeURL = url.split('://').shift() === 'https' ? https : http;
      let file = fs.createWriteStream(dest);
      typeURL.get(url, (response) => {
        response.pipe(file);
        file.on('finish', () => {
